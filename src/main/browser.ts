import { BrowserWindow, WebContentsView, session, ipcMain, type Session } from 'electron'
...
class BrowserShell {
  ...
  private wireTab(id: number, view: WebContentsView): void {
    ...
    const update = (): void => this.emit('tabUpdated', this.tabState(id))
  ...
  getState(): {
    profile: { id: string; client_name: string; service: string }
    tabs: TabState[]
    activeId: number
    chromeHeight: number
  } {
    return {
      profile: { id: this.profile.id, client_name: this.profile.client_name, service: this.profile.service },
...
export async function openProfileBrowser(profileId: string): Promise<void> {
  ...
  win.on('closed', () => {
    ...
    void (async () => {
      try {
        await pushSession(profileId, ses)
        await sb
          .from('profiles')
          .update({ in_use_by: null, in_use_by_email: null, in_use_by_name: null, in_use_at: null })
          .eq('id', profileId)
        await logAudit('salvou sessão', profileId, profile.client_name)
      } catch (e) {
